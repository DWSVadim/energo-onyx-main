...
function Apps() {
  ...
  const leftModalContent = [
    ...
    },
    {
      ...
      content: (
        <>
          Ответ:  Вам должно было уведомление вместе с последней квитанцией за электроэнергию. По этому мы с вами и связываемся чтобы вас об этом предупредить и подобрать с вами время чтобы к вам приехал мастер.
          <br />
          БЕЗ ПАУЗ,ГОВОРИМ ДАЛЬШЕ СВОЕ!!!
        </>
      )
          ...
          <div className="col-4">
            ...
            >
              ...
              <h5 style={{ textAlign: 'left', fontSize: '18px', lineHeight: '1.6', color: '#333' }}>
                ...
                <p>ФИО,ДОБРЫЙ ДЕНЬ/ВЕЧЕР! С ВАМИ СВЯЗЫВАЕТСЯ ЭНЕРГАСБЫТ. МЕНЯ ЗОВУТ (АНОСОВА СВЕТЛАНА АНДРЕЕВНА)
                  (АНОСОВ ВИТАЛИЙ АНДРЕЕВИЧ)</p>
                СВЯЗЫВАЮСЬ С ВАМИ КАСАТЕЛЬНО ЗАМЕНЫ СЧЕТЧИКА ПО ЭЛЕКТРОЭНЕРГИИ.
                ...
                <p>-ПОДСКАЖИТЕ, МАСТЕРА УДОБНЕЕ ПРИНЯТЬ СЕГОДНЯ ИЛИ В ДРУГОЙ ДЕНЬ?
                (ОТВЕТ)</p>

                <p>-ХОРОШО,В ТАКОМ СЛУЧАЕ ВОЗЬМИТЕ РУЧКУ И ЛИСТОЧЕК И ЗАПИШИТЕ ИМЯ ВАШЕГО МАСТЕРА КОТОРЫЙ БУДЕТ ВАМ УСТАНАВЛИВАТЬ СЧЕТЧИК И НОМЕР НОВОГО СЧЕТЧИКА.</p>

                <p>ЗАЧЕМ?
                -МАСТЕР ВАМ ПРЕДВАРИТЕЛЬНО ПОЗВОНИТ ПЕРЕД УСТАНОВКОЙ, ВЫ ЕМУ СКАЖЕТЕ НОМЕР СЧЕТЧИКА, ЧТОБЫ ОН СВЕРИЛ И УСТАНОВИЛ ИМЕННО ВАШ СЧЕТЧИК</p>
                ...
                <p>-ТАК ЖЕ, КОГДА МАСТЕР ПРИЕДЕТ МЕНЯТЬ ВАМ СЧЁТЧИК, ВАМ НЕОБХОДИМО БУДЕТ ПОДПИСАТЬ НОВЫЙ ДОГОВОР НА ОБСЛУЖИВАНИЕ С ЭНЕРГАСБЫТОМ. ДОГОВОР БУДЕТ В ДВУХ ЭКЗЕМПЛЯРАХ. ОДИН ОСТАЁТСЯ У ВАС, ОДИН У МАСТЕРА. ОН ЕГО ПЕРЕДАСТ ИНСПЕКТОРУ В ЭНЕРГАСБЫТ. ПЕРЕД ТЕМ КАК ДОПУСТИТЬ МАСТЕРА К УСТАНОВКЕ ВЫ ПРОВЕРЯЕТЕ УДОСТОВЕРЕНИЕ И СВЕРЯЕТЕ НОМЕР СЧЕТЧИКА С ТЕМ КОТОРЫЙ ВЫ ЗАПИСАЛИ, ОНИ ДОЛЖНЫ СОВПАДАТЬ.</p>

                <p>-ХОРОШО,В ДОГОВОР ТАКЖЕ НЕОБХОДИМО УКАЗАТЬ ОДИН ИЗ РЕГИСТРАЦИОННЫХ НОМЕРОВ ДЛЯ ТОГО ЧТОБЫ СЧЕТЧИК ЗАРЕГЕСТРИРОВАТЬ ИМЕННО ЗА ВАМИ.КАК ВАМ БУДЕТ УДОБНЕЙ ПО ПАСПОРТУ ИЛИ ПО СНИЛС?
                (ОТВЕТ)</p>

                <p>-ДАВАЙТЕ СВЕРИМ ДАННЫЕ ПО ДОГОВОРУ,ВЫ (ФИО),ДАТА РОЖДЕНИЯ...</p>

                <p>Так же для подключение льготы на оплату электроэнергии подскажите сколько по вашему адресу проживает людей в зависимости от количества проживающих людей в вашей квартире будет зависеть размер скидки на оплату электроэнергии
                (ОТВЕТ)</p>

                <p>-ВСЕ ЗАЯВКА УСПЕШНО ЗАРЕГЕСТРИРОВАНА,ОЖИДАЙТЕ МАСТЕРА.</p>
